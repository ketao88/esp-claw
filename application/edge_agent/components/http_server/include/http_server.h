...
typedef struct {
    bool llm_error_active;
    char llm_error_message[256];
} http_server_llm_status_t;

typedef esp_err_t (*http_server_get_llm_status_fn)(http_server_llm_status_t *status);
...
typedef struct {
    ...
    http_server_get_llm_status_fn get_llm_status;
