...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    /* LLM status */
    if (ctx->services.get_llm_status) {
        http_server_llm_status_t llm = {0};
        if (ctx->services.get_llm_status(&llm) == ESP_OK) {
            cJSON_AddBoolToObject(root, "llm_error_active", llm.llm_error_active);
            if (llm.llm_error_message[0]) {
                http_server_json_add_string(root, "llm_error_message", llm.llm_error_message);
            }
        }
    }
