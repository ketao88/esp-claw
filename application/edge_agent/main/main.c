...
#if CONFIG_APP_CLAW_CAP_TIME
static esp_err_t main_get_llm_status(http_server_llm_status_t *status)
{
    if (!status) {
        return ESP_ERR_INVALID_ARG;
    }
    memset(status, 0, sizeof(*status));
    status->llm_error_active = cap_status_indicator_is_llm_error();
    const char *err_msg = cap_status_indicator_last_error();
    if (err_msg) {
        strlcpy(status->llm_error_message, err_msg, sizeof(status->llm_error_message));
    }
    return ESP_OK;
}
#endif
...
#endif
#if CONFIG_APP_CLAW_CAP_TIME
            .get_llm_status = main_get_llm_status,
#endif
        },
    }));
