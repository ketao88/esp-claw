...
static bool s_sta_forever_retry;
...
static esp_timer_handle_t s_periodic_retry_timer;
...
static void periodic_retry_timer_cb(void *arg);
static bool wifi_manager_verify_ap_exists(void);
                               ...
                               void *event_data)
{
    ...
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_DISCONNECTED:
            ...
            if (s_retry_count < WIFI_RETRY_MAX_COUNT || s_sta_forever_retry) {
                s_retry_count++;
                s_mode = s_ap_active ? WIFI_MODE_APSTA_TRYING : s_mode;
                ESP_LOGI(TAG, "STA disconnected, retry %d/%d in %dms",
                         s_retry_count, WIFI_RETRY_MAX_COUNT, WIFI_RETRY_INTERVAL_MS);
                if (s_reconnect_timer) {
                    esp_timer_stop(s_reconnect_timer);
                    esp_timer_start_once(s_reconnect_timer, (uint64_t)WIFI_RETRY_INTERVAL_MS * 1000ULL);
                } else {
                    esp_wifi_connect();
                }
            } else {
                ESP_LOGE(TAG, "STA failed after %d retries, starting periodic retry", WIFI_RETRY_MAX_COUNT);
                s_sta_forever_retry = true;
...
static void reconnect_timer_cb(void *arg)
{
    ...
    /* Verify AP is still in range before attempting reconnect */
    if (s_retry_count > 1 && !wifi_manager_verify_ap_exists()) {
        ESP_LOGW(TAG, "Target AP '%s' not found in scan, skipping reconnect", s_sta_ssid);
        return;
    }
...
{
    (void)arg;
    if (!s_sta_forever_retry) {
        return;
    }
    if (!wifi_manager_verify_ap_exists()) {
        ESP_LOGD(TAG, "Periodic retry: target AP not found, will check again later");
        return;
    }
    ESP_LOGI(TAG, "Periodic retry: target AP '%s' found, attempting reconnection", s_sta_ssid);
    s_sta_configured = true;
    s_retry_count = 0;
    esp_err_t err = esp_wifi_set_mode(WIFI_MODE_APSTA);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Periodic retry: set_mode APSTA failed: %s", esp_err_to_name(err));
        return;
    }
    s_mode = WIFI_MODE_APSTA_TRYING;
    err = esp_wifi_connect();
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Periodic retry: esp_wifi_connect failed: %s", esp_err_to_name(err));
    }
}
...
static bool wifi_manager_verify_ap_exists(void)
{
    ...
    wifi_ap_record_t ap_records[1];
    uint16_t record_count = 1;
    if (esp_wifi_scan_get_ap_records(&record_count, ap_records) != ESP_OK) {
        return false;
    }
...
esp_err_t wifi_manager_init(void)
{
    ...
    const esp_timer_create_args_t periodic_args = {
        .callback = periodic_retry_timer_cb,
        .name = "wifi_periodic_retry",
    };
    ESP_ERROR_CHECK(esp_timer_create(&periodic_args, &s_periodic_retry_timer));
    ESP_ERROR_CHECK(esp_timer_start_periodic(s_periodic_retry_timer, 300 * 1000 * 1000ULL)); /* every 5 min */
...
esp_err_t wifi_manager_apply_sta_config(const wifi_manager_config_t *config)
{
    ...
    /* Small delay for WiFi driver to complete disconnect state transition */
