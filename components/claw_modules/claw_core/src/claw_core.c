...
/* Hard limit: keep at most 4 rounds (8 messages) of tool-call history.
   Prevents unbounded context growth that wastes tokens. */
#define MAX_HISTORY_ROUND 4
#define MAX_HISTORY_MESSAGES (MAX_HISTORY_ROUND * 2)
                                         ...
                                         size_t obs_providers_csv_size)
{
    ...
    *out_tools_json = NULL;
    ...
    for (i = 0; i < s_core->context_provider_count; i++) {
        ...
        /* ON-DEMAND TOOLS: skip tool definitions on first call unless user text
           contains hardware control keywords. This saves 200-400 tokens per request. */
        if (context.kind == CLAW_CORE_CONTEXT_KIND_TOOLS
                && (!runtime_messages || cJSON_GetArraySize((cJSON *)runtime_messages) == 0)) {
            const char *hw_keywords[] = {
                "开灯", "关灯", "灯", "GPIO", "gpio", "UART", "uart",
                "I2C", "i2c", "SPI", "spi", "ADC", "adc",
                "传感器", "温度", "湿度", "电压", "电流",
                "PWM", "pwm", "读", "写", "设置", "控制",
                "logic", "logic_analyzer", "逻辑分析",
                "马达", "motor", "舵机", "servo",
                "LED", "led", "屏幕", "display", "LCD",
                "pin", "PIN", "IO", "io", "触发", "输出"
            };
            bool skip_tools = true;
            const char *user_text = request->view.user_text;
            for (size_t kw = 0; kw < sizeof(hw_keywords) / sizeof(hw_keywords[0]); kw++) {
                if (user_text && strstr(user_text, hw_keywords[kw])) {
                    skip_tools = false;
                    break;
                }
            }
            if (skip_tools) {
                ESP_LOGD(TAG, "skip_tools: no hardware intent detected in user text");
                free(context.content);
                continue;
            }
        }
...
static void claw_core_task(void *arg)
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            /* Truncate runtime_messages: keep at most MAX_HISTORY_ROUND tool rounds.
               Prevents unbounded context growth across iterations. */
            while (runtime_messages && cJSON_GetArraySize(runtime_messages) > MAX_HISTORY_MESSAGES) {
                cJSON_DeleteItemFromArray(runtime_messages, 0);
            }
...
finish_request:
        ...
        if (err != ESP_OK) {
            ...
            /* Notify completion observers on error too */
            if (s_core->completion_observer_count > 0) {
                claw_core_completion_summary_t summary = {
                    .request_id = request.view.request_id,
                    .session_id = request.view.session_id,
                    .final_text = NULL,
                    .error_message = response.view.error_message ? response.view.error_message : esp_err_to_name(err),
                    .context_providers_csv = obs_providers_csv,
                    .tool_calls_csv = obs_tool_calls_csv,
                };
                for (size_t i = 0; i < s_core->completion_observer_count; i++) {
